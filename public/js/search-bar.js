...
(function () {
  ...
  /* Monday-first column for a given date. */
    ...
    for (var i = 0; i < MONTHS_AHEAD; i++) {
      ...
      for (var d = 1; d <= daysIn; d++) {
        ...
        day.setAttribute("aria-label",
          d + " " + MONTHS[m.getMonth()] + " " + m.getFullYear());
  ...
  /* Paints selection, the range pill and the pill's rounded ends. A pill end
     is rounded where the range stops, and also at the two edges of a row so a
     range spanning several weeks reads as one bar per row. */
  function paint() {
    ...
    // The pill spans the nights, so it runs from arrival up to but not
    // including checkout; a one night stay is a single rounded cell.
    var lastNight = hi ? new Date(hi.getFullYear(), hi.getMonth(), hi.getDate() - 1) : null;

    var cells = scroll.querySelectorAll(".ch-pick__cell");
    Array.prototype.forEach.call(cells, function (cell) {
      cell.classList.remove("is-pill", "is-start", "is-end",
                            "is-pill-left", "is-pill-right");
    ...
    whenUI.value.textContent = label();
  ...
  function syncMark() {
    ...
    monthBlocks.forEach(function (b, i) {
      ...
      if (bBottom > top + 4 && bTop < bottom - 4) {
        if (first === -1) first = i;
        last = i;
      }
    });
  ...
  function stepButton(kind, rowName) {
    ...
    b.setAttribute("aria-label",
      (kind === "plus" ? "Add " : "Remove ") + rowName.toLowerCase());
  ...
  // Empty dates would otherwise ride along as bare `from=&to=`.
